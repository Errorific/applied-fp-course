...
data ConfigError
  = BadConfFile DecodeError | ReadException IOError | NoPort | NoDbFile
  deriving Show
...
partialConfDecoder = D.withCursor $ \c -> do
  o <- D.down c
  confPort <- D.fromKeyOptional "port" D.integral o
  dbFile <- D.fromKeyOptional "dbFile" D.string  o
  pure $ PartialConf (Last . Port <$> confPort) (Last . DBFilePath <$> dbFile)
