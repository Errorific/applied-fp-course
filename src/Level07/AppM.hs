...
instance Monad (AppM e) where
  ...
  (>>=) (AppM a) f = AppM (\env -> a env >>= either (pure . Left) (\x -> (runAppM $ f x) env))
...
instance MonadError e (AppM e) where
  ...
  catchError (AppM a) r = AppM (\env -> a env >>= either (\x -> (runAppM $ r x) env) (pure . pure))
